...
export default class ParseTool {
  ...
  interfaceProperties(node: ts.InterfaceDeclaration): Array<{name: string, type: {code: string, doc: string }}> {
    return node.members
      .filter(tsh.isPropertySignature)
      .map((member: ts.TypeElement): {name: string, type: {code: string, doc: string}} => ({
  ...
  typeAliasLiterals(node: ts.TypeAliasDeclaration): Array<string|number> {
    ...
    return node.type.types.map((type: ts.TypeNode): string|number => {
  ...
  typeAliasTypes(node: ts.TypeAliasDeclaration, namespace?: string): Array<string> {
    ...
    return node.type.types.map((type: ts.TypeNode): string => {
